...
public class IsABinarySearchTree {
    ...
    public static BstPair getBST(Node root)
    {
        if(root==null)
        {
        ...
        BstPair lp=getBST(root.left);
        BstPair rp=getBST(root.right);
        if(lp.isBst==true && rp.isBst==true && lp.max<root.data && root.data<rp.min)
        {
            ...
            BstPair myPair=new BstPair(true,max,min);
            return myPair;
        }
        else
        {
            BstPair p=new BstPair(false,-22,34);
            //some opposite values
            return p;
        }
    }
    ...
    public static void main(String[] args) throws Exception {
        ...
        System.out.println(ansPair.isBst);
