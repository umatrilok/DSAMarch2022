...
public class AreTreesMirrorInShape {
    ...
    //preorder//our work is happening at top
    public static boolean areMirror(Node n1, Node n2) {
        if(n1.children.size()!=n2.children.size()) return false;
        int i=0;
        int j=n1.children.size()-1;
        //not i<j//it we only check half, we want to check full//in case of not mirror trees
        while(i<n1.children.size())
        {
            boolean ans = areMirror(n1.children.get(i),n2.children.get(j));
